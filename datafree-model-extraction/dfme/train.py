...
def train(args, teacher1, teacher2, student, generator, device, optimizer, epoch):
...
def main():
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            ...
            correct += pred.eq(target.view_as(pred)).sum().item()
    accuracy = 100. * correct / len(test_loader.dataset)
    print('\nTeacher combined - Test set: Accuracy: {}/{} ({:.4f}%)\n'.format(correct, len(test_loader.dataset),accuracy))
    correct = 0
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
            ...
            correct += pred.eq(target.view_as(pred)).sum().item()
    accuracy = 100. * correct / len(test_loader.dataset)
    print('\nTeacher 1 - Test set: Accuracy: {}/{} ({:.4f}%)\n'.format(correct, len(test_loader.dataset),accuracy))
    correct = 0
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
    ...
    args.teacher1 = teacher1
    args.teacher2 = teacher2
    ...
    for epoch in range(1, number_epochs + 1):
        ...
        train(args, teacher1=teacher1, teacher2=teacher2, student=student, generator=generator, device=device, optimizer=[optimizer_S, optimizer_G], epoch=epoch)
        # Test

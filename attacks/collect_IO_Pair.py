...
# def main():
#     num_samples = 1000  # Define the number of samples to test
...
async def query_model(session, data):
    ...
    # Send a POST request to the API server
    async with session.post(url, data=input_json, headers={"Content-Type": "application/json"}) as response:
...
async def main():
    ...
    batch_size = 10  # Define the batch size for testing
    io_pairs = await collect_io_pairs(test_loader, num_samples, batch_size)  # Collect input-output pairs
    analyze_accuracy(io_pairs)  # Analyze the model's accuracy
    save_to_json(io_pairs, "io_pairs_cifar10.json")  # Save results to a JSON file
